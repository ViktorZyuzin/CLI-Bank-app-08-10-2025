...
class BankService {
    ...
    showTransactionHistory() {
        const transactions = this.currentUser.getLastTransactions(10);
        
        if (transactions.length === 0) {
            console.log('История операций пуста');
            return;
        }

        console.log('\n--- Последние операции ---');
        transactions.forEach((transaction, index) => {
            const type = transaction.type === 'deposit' ? 'Пополнение' : 'Снятие';
            const sign = transaction.type === 'deposit' ? '+' : '-';
            const date = transaction.date.toLocaleString('ru-RU');
            console.log(`${index + 1}. ${date} | ${type} | ${sign}${transaction.amount} руб. | Баланс: ${transaction.balanceAfter} руб.`);
        });
    }

    showAccountInfo() {
        const userInfo = this.currentUser.getUserInfo();
        console.log('\n--- Информация о счете ---');
        console.log(`Владелец: ${userInfo.name}`);
        console.log(`Логин: ${userInfo.login}`);
        console.log(`Номер счета: ${userInfo.account.accountNumber}`);
        console.log(`Текущий баланс: ${userInfo.account.balance} руб.`);
        console.log(`Дата открытия: ${userInfo.account.createdAt.toLocaleDateString('ru-RU')}`);
        console.log(`Всего операций: ${userInfo.account.totalTransactions}`);
    }
